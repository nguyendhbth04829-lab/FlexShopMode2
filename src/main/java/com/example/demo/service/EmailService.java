...
public class EmailService {
     ...
     */
    public boolean guiEmailOtpDangKy(String toEmail, String maOtp, int thoiGianHieuLucGiay, String hoVaTen) {
        String tenHienThi = (hoVaTen != null && !hoVaTen.isBlank()) ? hoVaTen.trim() : "Quý khách";
        log.info("==================================================================");
        log.info(">>> [FLEXSHOP OTP ĐĂNG KÝ] Gửi mã xác thực đến: {}", toEmail);
        log.info(">>> MÃ OTP ĐĂNG KÝ (6 SỐ): [ {} ]", maOtp);
        log.info(">>> THỜI GIAN HIỆU LỰC: {} GIÂY (5 PHÚT)", thoiGianHieuLucGiay);
        log.info("==================================================================");

        if (mailSender == null) {
            log.warn("JavaMailSender chưa được khởi tạo. Bỏ qua gửi SMTP.");
            return false;
        }
        ...
        java.util.concurrent.CompletableFuture.runAsync(() -> {
            try {
                ...
                helper.setFrom(fromEmail, "FlexShop Enterprise");
                helper.setTo(toEmail);
                helper.setSubject("🔒 [" + maOtp + "] Mã OTP xác thực đăng ký tài khoản FlexShop");

                String htmlContent = """
                    <div style="font-family: Arial, sans-serif; max-width: 580px; margin: auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff;">
                        <div style="text-align: center; margin-bottom: 24px;">
                            <h2 style="color: #4f46e5; margin: 0; font-weight: 800;">FlexShop Enterprise</h2>
                            <span style="color: #64748b; font-size: 13px;">Sàn Thương Mại Điện Tử Đa Kênh FlexShop</span>
                        </div>
                        <div style="padding: 20px; background-color: #f8fafc; border-radius: 12px; margin-bottom: 20px;">
                            <p style="color: #1e293b; font-size: 15px; margin-top: 0;">Xin chào <strong>%s</strong>,</p>
                            <p style="color: #475569; font-size: 14px; line-height: 1.6;">
                                Cảm ơn bạn đã đăng ký tài khoản tại <strong>FlexShop</strong> với địa chỉ email <strong>%s</strong>.
                            </p>
                            <p style="color: #475569; font-size: 14px; margin-bottom: 8px;">Dưới đây là mã xác thực OTP kích hoạt tài khoản của bạn:</p>
                            <div style="text-align: center; margin: 20px 0;">
                                <span style="display: inline-block; font-size: 32px; font-weight: 800; letter-spacing: 8px; color: #4f46e5; background: #ede9fe; padding: 12px 28px; border-radius: 12px; border: 2px dashed #818cf8;">
                                    %s
                                </span>
                ...
                """.formatted(tenHienThi, toEmail, maOtp);

                helper.setText(htmlContent, true);
                mailSender.send(message);
                log.info(">>> [FLEXSHOP OTP ĐĂNG KÝ] ĐÃ GỬI THÀNH CÔNG EMAIL ĐẾN: {}", toEmail);
            } catch (Exception e) {
                log.error(">>> [FLEXSHOP OTP ĐĂNG KÝ] LỖI GỬI EMAIL QUA SMTP GMAIL: {}", e.getMessage());
            }
        });
